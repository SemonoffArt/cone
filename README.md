...
Запрос на автоматизацию расчёта объёма и массы конусов руды поступил от **директора фабрики** и **главного металлурга**. 

Цель приложения — дать технологам, диспетчерам и руководству **быстрый и наглядный инструмент** для оценки запасов руды на складах без выезда на объект и без проведения сложных ручных измерений.
...
Параметры камер задаются в `utils/constants.py`:
...
│   ├── canvas_view.py        # Холст с изображением и треугольником
│   ├── info_panel.py         # Информационная панель
│   └── toolbar.py / menu.py  # Панель инструментов и меню
├── utils/
│   ├── constants.py          # Константы и параметры камер
